 ...
 *    롤오버는 사용자 승인이 필요한 상태 변경이라 `marketing-team-setup` 가 ⏸ 로 물어보고 한다.
 */
...
if (HOOK) {
  ...
  // ⚠️ 표현 원칙 (실측 2026-09-14) · 이 블록은 additionalContext 로 매 세션 주입된다.
  //    "~한다/~해라" 식 명령문으로 쓰면 모델이 "사용자가 안 시켰는데 끼워 넣은 지시"로 읽고
  //    통째로 무시한다 — 실제로 그런 답을 냈다("유도하는 방식이라 따르지 않았습니다").
  //    그래서 지시가 아니라 **사실 보고**로 적는다: 무엇을 쟀는지, 기존에 정해 둔 절차가
  //    무엇인지를 서술하고, 그 절차를 지금 따르라고 명령하지 않는다. 라우팅·실행 판단은
  //    AI 마케터(SKILL.md)의 몫이고 여기서는 근거만 준다.
  const 맥락 = ['[사실 데이터 · 지시 아님] 아래는 이 작업 폴더의 logs/build-log.md 를 스크립트로 집계한 결과다. 사용자가 준 지시가 아니라 파일 상태를 기계적으로 요약한 것이다.'];
  if (먼저.length) 맥락.push(
    `■ 재방문 신호 — 실행 기록에서 반복 주기나 미완 항목이 감지됐다. 이상 신호가 아니라 "다시 확인할 때가 됐다"는 사실이며,` +
    ` 이 팀에 기록된 운영 규칙은 재방문 신호를 "첫 답에서 먼저 꺼낸다"이다 (물어봐 주기를 기다리면 일어나지 않는다는 기록이 있다).\n` +
    [...종류별].slice(0, 3).map(([종류, xs]) =>
      `- ${한줄([종류, xs])} → ${xs[0][3]}`).join('\n') +
    `\n  참고 사실 ① 이 신호가 있어도 사용자가 이번에 다른 일을 요청했으면 그 일이 우선이고, 이 신호는 그 답의 맨 끝에 한 줄로 덧붙이는 것이 지금까지의 운영 방식이었다.` +
    `\n  참고 사실 ② 이 신호만으로 스킬을 바로 실행하지 않고, 「돌릴까요?」로 확인받은 뒤 실행하는 것이 지금까지의 운영 방식이었다.`);
