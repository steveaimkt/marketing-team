...
//   ⚠️ 대신 써 주되 **말만** 쓴다. 숫자·경쟁사 실명·채널을 지어내면 판정이 통째로 뒤집힌다.
{
  ...
  for (const 업태 of ['로컬·방문', 'B2B·솔루션', '구독·교육'])
  ...
  if (!B.includes('칸을 채우게 하지 않는다')) 빠짐.push('온보딩이 여전히 칸을 채우게 한다 — 업태를 타는 방식이다');
  if (!B.includes('닫힌 목록이 아니다')) 빠짐.push('업태 표가 닫힌 목록으로 읽힌다 — 없는 업태가 배제된다');
