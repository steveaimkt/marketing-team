...
|---|---|---|
| Phase 1 | 065 산출물이 없으면 ⏸ **"고객 등급 분석 결과가 없습니다. 등급 분석을 먼저 하시겠어요?"**를 묻는다 | 구매 분포(구매액) 직접 입력 + `[추정]` 모드 |
