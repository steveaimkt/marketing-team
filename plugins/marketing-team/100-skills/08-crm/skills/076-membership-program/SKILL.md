---
id: "076"
name: 멤버십 등급 설계
description: "등급 혜택 구조 + 마진 시뮬레이션. 고객 등급제·멤버십 혜택 구조를 수익성 있게 설계해야 할 때 사용."
slug: membership-program
category: 08-crm
tier: EXPERT
triggers:
  - "멤버십 만들자"
  - "고객 등급제 설계해줘"
  - "VIP 혜택 구조 짜줘"
  - "등급별 혜택이 남는 장사인지 계산해줘"
inputs: [065 세그먼트 리포트(권장) 또는 고객 구매 분포, 마진 구조(원가율·배송비), 목표 리텐션]
sample_fallback: sample-data/A브랜드-고객마스터.csv   # `inputs/` 를 먼저 보고, 없으면 **묻지 않고 바로** 이 파일로 완주한다 (산출물에 [샘플])
outputs: [등급 구조표(기준·혜택), 등급별 마진 시뮬레이션, 승급·강등·유지 규칙, 저장 파일(.xlsx + .html + .md)]
requires: [brand/profile.md]
chains_to: ["065"]
gate: false
mutating: false
writes_to: [outputs/{날짜}/076-membership-program/076-membership-program.xlsx, outputs/{날짜}/076-membership-program/076-membership-program.html, outputs/{날짜}/076-membership-program/076-membership-program.md]
builder: 사용자 (이 회사)
version: 1.0
persona: "로열티 프로그램 설계 10년차, 마진 시뮬레이션 없이는 혜택을 약속하지 않는다"
when_to_use: "고객 등급제·멤버십 혜택 구조를 수익성 있게 설계해야 할 때"
success_metrics: [등급별 마진율, 재구매율 개선폭, 상위 등급 승급률]
---

# 076 멤버십 등급 설계

> 065 고객 등급 분석 세그먼트의 매출 기여 분포로 등급 경계를 정하고, 혜택 원가를 반영한 마진 시뮬레이션으로 "퍼주다 망하는 멤버십"을 설계 단계에서 차단한다.
> 계승: 065 고객 등급 분석의 세그먼트 정의표(M 분위·매출 기여) → 등급 경계 근거 / 016 묶음 상품 설계의 할인 후 실마진 시뮬레이션 공식 재사용.

> **내 일이 아닌 것**: **반복 구매**의 등급·혜택이다. 한 번 사는 묶음은 016, 사람을 모으는 운영은 036.
> 나누는 말은 `docs/헷갈리는-쌍.md` 에 있다.

## Contract
1. 등급 경계는 실제 구매 분포 데이터(065 세그먼트 리포트의 M 분위·매출 기여)로 정한다. "6개월 50만원" 같은 감 기준을 업종 불문 적용하지 않는다. 데이터 없으면 [추정] 태그 필수.
2. 모든 혜택은 원가로 환산되어 등급별 실마진 시뮬레이션에 반영된다 (실마진 = 등급 매출 × 마진율 − 혜택 원가 합계). 원가 미산정 혜택은 구조표에 넣지 않는다.
3. 등급별 혜택 원가율이 해당 등급 마진을 초과하는 설계는 전달하지 않는다. 초과 시 혜택 축소안을 함께 제시한다.
4. 승급 규칙과 함께 강등·유지 규칙(산정 주기·유예)을 반드시 설계한다. 승급만 있는 제도는 미완성이다.
5. 같은 구매 분포를 다시 넣으면 같은 등급 경계가 나온다 (경계 산출 기준을 산출물에 명시).

## 게이트 (대화형에서만 멈춘다)

> 사람이 부르면 아래에서 멈춰 답을 받는다. 대시보드·cron 에서는 기본값으로 완주하고
> 산출물에 "물어보지 못해 기본값으로 갔다"를 밝힌다.

| 멈추는 곳 | 묻는 것 | 기본값 (답 없으면) |
|---|---|---|
| Phase 1 | 065 산출물이 없으면 ⏸ 아래 「질문 065 먼저」 그대로 | 구매 분포(구매액) 직접 입력 + `[추정]` 모드 |
| Phase 2 | 등급은 **3~4개** · 5등급 이상은 만들지 않는다 | 3등급 (VIP · GOLD · 일반) |
| Phase 5 뒤 | 혜택 원가율이 마진을 넘으면 **그 등급을 제안하지 않는다** | 혜택을 줄이거나 등급 경계를 올린다 |

⛔ **Phase 1 질문은 AI-마케터의 `[실행 계획]` 보다 먼저, 그 계획과 분리된 제 turn 으로 낸다.**
065 산출물 유무는 등급을 무엇 하나로 나누는지(구매액만 vs RFM)를 가르는 결정이라 계획 각주로
접어 넣지 않는다. **자유 문장으로 답**을 받는다 — "그대로 진행해줘." 처럼 `진행 승인` 문구가
아니어도 이 질문의 답으로는 유효하다. 답을 받은 뒤에야 그 결과를 반영한 `[실행 계획]`을 내고
`[승인 요청]`으로 정확한 "진행 승인"을 받는다 (실측 2026-09-14, 이 질문이 계획 화면 안
각주로 접혀 들어가 "그대로 진행해줘."가 거부되고 정확한 "진행 승인" 문구를 다시 요구했다).

### 확인 질문 (그대로 낸다)

`[실행 계획]` 보다 먼저, 이 한 줄만 제 턴으로 낸다. 앞뒤에 계획이나 안내를 붙이지 않는다.

```질문 065 먼저
고객 등급 분석 결과가 없습니다. 등급 분석(065)을 먼저 하시겠어요?
```

## Phases
1. **브랜드·데이터 로드**: brand/profile.md(마진율·AOV·구매주기) 자동 참조. 065 세그먼트 리포트가 있으면 세그먼트 정의표·매출 기여를 등급 재료로 로드. 없으면 **`[실행 계획]`을 내기 전에** ⏸ 게이트 아래 「질문 065 먼저」를 그대로 제 turn으로 묻는다(자유 문장 답, "진행 승인" 문구를 요구하지 않는다). "먼저 한다"고 답하면 065를 실행해 최근 구매일·구매 빈도까지 반영해 등급을 나눈다. "그대로 진행"이라고 답하면 구매액 한 가지 분위만으로 나눈다. 답이 없으면 구매액 분위 기준으로 진행하고 `[추정]`을 붙인다. 이 답이 정해진 뒤에 그 경로를 반영한 `[실행 계획]`을 내고 `[승인 요청]`으로 "진행 승인"을 받는다.
2. **등급 구조 설계**: 3~4등급 (5등급 이상 금지). 경계는 산정 기간(최근 6~12개월) 구매액 분위로: 예) VIP = 상위 5% / GOLD = 상위 20% / 일반. 065 세그먼트와 매핑 (챔피언→VIP, 충성→GOLD)해 등급별 예상 인원·매출 기여를 산출.
3. **혜택 설계**: 등급별 차등표 (할인율·적립률·무료배송·전용 혜택·선공개). 상위 등급일수록 "비용형 혜택(할인)"보다 "경험형 혜택(선공개·전용)"의 비중을 높이는 원칙 적용.
4. **마진 시뮬레이션**: 016 공식 재사용, 등급별로:
   `혜택 원가 = 예상 매출 × (할인율 + 적립률 × 사용률) + 무료배송 원가 × 주문 수 + 전용 혜택 원가`
   `실마진 = 등급 매출 × 기존 마진율 − 혜택 원가` → 등급별 혜택 원가율(%)과 손익 표. 리텐션 개선 가정(보수/기본/낙관 3안)으로 회수 기간 제시.
5. **승급·강등·유지 규칙**: 산정 주기(분기/반기), 강등 유예(1주기), 등급 통보 시점·채널. 등급 산정 로직은 한 문단으로 명문화 (CS 분쟁 대비).
6. ⏸ **구조 확정**: 시뮬레이션 3안 중 사용자가 선택·조정 → 최종 구조표 인도.
7. **체인 제안**: 065 고객 등급 분석 (분기마다 재실행해 등급 인원 이동·경계 리밸런싱).

8. **파일로 남긴다**: 위 산출물을 아래 「⛔ 착지」의 세 파일로 저장하고 경로를 알린다. 화면에만 띄우고 끝내지 않는다. 마케터가 다음 날 다시 열 수 있어야 한다.
   > 쓰기 권한이 없으면 **실패로 처리하지 않는다.** 산출물은 그대로 화면에 내고 맨 아래에 "`outputs/{날짜}/076-membership-program/076-membership-program.md` 로 저장하려 했으나 권한이 없어 남기지 못했습니다" 를 적는다. 못 한 일을 못 했다고 말하는 것도 산출물의 일부다.

> ⛔ **착지 · 세 파일로 쓴다**
> · 표 → `outputs/{날짜}/076-membership-program/076-membership-program.xlsx`
> · 도식·차트 → `outputs/{날짜}/076-membership-program/076-membership-program.html`
> · 해설 → `outputs/{날짜}/076-membership-program/076-membership-program.md`
> 경로를 새로 만들지 않는다. 위 줄을 그대로 쓰고 `{날짜}` 만 오늘로 바꾼다.
> 아티팩트·스크래치패드·화면 출력은 착지가 아니다. **파일이 없으면 안 한 것이다.**
> **형식** · `.xlsx` 는 openpyxl 로 만든다 · 첫 시트는 「요약」, 표마다 시트 하나 · 머리 행 굵게 + 배경 `EBEBEB` · 틀 고정 A2 · 자동 필터 · **수는 수로 넣는다**("1,240" 은 합계가 안 돈다) (`docs/공통규약.md §H`)
> 🔴 **`.xlsx` 는 우리가 직접 만들지 않는다.** 표 내용은 여기서 만들고 **파일로 바꾸는 일만 앤트로픽 공식 `document-skills` 의 xlsx 스킬**에 넘긴다.
> 안 깔려 있으면 **`.csv` 로 내고 그렇게 말한다.** 설치 명령은 `/plugin marketplace add anthropics/skills`, `/plugin install document-skills@anthropic-agent-skills` 이다. ⛔ 설치를 강요하지 않는다.

## Output Format · **파일에 들어갈 내용**

⛔ **아래는 `outputs/{날짜}/076-membership-program/076-membership-program.md` 안에 들어갈 것이다. 화면에 그대로 쓰는 것이 아니다.**
파일에 쓰고 나서, 화면에는 **주요 표 하나 · 경로 · 결론 3줄 · 부족한 것**을 낸다 (표 말고는 15줄 이내).
```
## 멤버십 설계 — {브랜드} · 산정 기간 {기간} · {날짜}
결론 요약: {n}등급 구조 · 총 혜택 원가율 {%} · 손익분기 리텐션 개선 {+x%p}

### ① 등급 구조표
| 등급 | 기준 (구매액 분위) | 예상 인원 | 매출 기여 | 065 세그먼트 매핑 |
|---|---|---|---|---|

### ② 혜택 차등표
| 등급 | 할인 | 적립 | 배송 | 경험형 혜택 |
|---|---|---|---|---|

### ③ 마진 시뮬레이션 (등급별)
| 등급 | 예상 매출 | 혜택 원가 | 혜택 원가율 | 실마진 |
|---|---|---|---|---|
(검증: 전 등급 실마진 > 0 ✅ / 초과 등급은 축소안 병기)

### ④ 도식 → `076-membership-program.html`
`docs/차트-규격.md` §3-2 ⑩ 등급 피라미드를 따른다.
층마다 **인원과 매출 기여를 함께** 적는다 — 인원이 적은데 매출이 큰 층이 보여야 설계가 읽힌다.
층 색은 `--olive` 의 `fill-opacity` 를 위 1.0 → 아래 0.35 로 내린다. **색상을 바꾸지 않는다.**
⛔ 등급을 표로만 내지 않는다. 몇 층인지·어느 층이 두꺼운지는 표로 안 보인다.

### ⑤ 운영 규칙: 산정 {주기} · 강등 유예 {기간} · 통보 {채널}

### ⑥ 구조 확정 ⏸ (보수/기본/낙관 3안 중 선택)
다음 액션: 분기마다 065 고객 등급 분석을 다시 실행해 등급 인원 이동과 경계를 리밸런싱하세요.

저장 파일
· 해설 outputs/{날짜}/076-membership-program/076-membership-program.md
· 도식 outputs/{날짜}/076-membership-program/076-membership-program.html
```

## Anti-Patterns
- **감으로 등급 경계**: "왠지 50만원이 VIP"는 인원이 0.5%거나 30%로 튄다. 반드시 분포 분위에서 도출.
- **혜택 원가 미계산**: 적립률 5%가 마진 15% 업종에서 어떤 의미인지 계산 없이 확정하는 것. 적립은 사용률까지 곱해야 진짜 원가다.
- **승급만 설계**: 강등 규칙이 없으면 2년 뒤 전원 VIP가 된다. 강등·유예·통보까지가 한 세트.
- **등급 인플레이션**: 5~7등급으로 쪼개면 등급 간 혜택 차가 미미해져 승급 동기가 죽는다. 3~4등급 상한.
- **전 등급 할인 중심 혜택**: 상위 등급까지 할인으로만 채우면 멤버십이 상시 세일이 된다. 상위는 경험형 혜택 비중을 높인다.

## 실습해 보기

```
멤버십 등급제 설계해줘
```

**등급은 3~4개까지다.** 5등급을 만들면 고객이 자기 등급을 기억하지 못하고,
혜택 원가만 늘어난다. 마진을 넘는 혜택은 제안하지 않는다.

## 활용
- **D2C 멤버십 런칭**: 065 리포트의 챔피언·충성 세그먼트 규모로 VIP·GOLD 경계를 잡고, 시뮬레이션 기본안으로 3개월 파일럿 후 리밸런싱.
- **기존 등급제 수익성 점검**: 이미 운영 중인 등급제의 혜택 원가율을 ③표로 재계산해 적자 등급을 찾아 혜택 재배치.
- **VIP 전용 캠페인 연계**: 등급 구조표를 074 이메일 발송 설계(등급별 오퍼 분리)·075 카카오 알림 설계(승급 축하·강등 예고)의 타겟 기준으로 전달.
