...
설치하면 **Skills 탭에 4개, Agents 탭에 15명**이 뜬다. 그다음 한마디 한다.
...
작업할 폴더를 하나 열고 「팀 점검하자」. 그 폴더에 `brand/` · `outputs/` · `logs/` 를 만들어 준다.
앞으로 산출물은 전부 그 폴더에 쌓인다.

### ③ 폴더를 열어서 · 클로드 코드 전용

저장소를 클론해 폴더를 열어도 된다. `.claude/agents` · `.claude/skills` 가 상위 폴더를 가리키도록
