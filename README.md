...
| **②** | **`마케팅팀-시작하기`** 로 우리 회사 정보 | 100개가 **우리 값으로** 돕니다. 3분 |
...
| 스킬 | 언제 |
|---|---|
| `마케팅팀-시작하기` | 설치 직후 점검 · 작업 폴더 생성 · 브랜드 정보 등록 |
...
작업할 폴더를 하나 열고 **`마케팅팀-시작하기`**. 그 폴더에 `brand/` · `outputs/` · `logs/` 를 만들어 준다.
앞으로 산출물은 전부 그 폴더에 쌓인다.

### ③ 폴더를 열어서 · 클로드 코드 전용

저장소를 클론해 폴더를 열어도 된다. 열고 나서 **`마케팅팀-시작하기`**. `.claude/agents` · `.claude/skills` 가
상위 폴더를 가리키도록 연결돼 있어 그대로 뜬다. 스킬 파일과 산출물이 **한 폴더에 다 보이는** 것이
장점이라 책 실습에는 이쪽이 낫다.
...
> 그러면 담당이 하나도 안 걸린다. `마케팅팀-시작하기` 가 그것을 찾아 그 자리에서 고친다.
...
plugins/marketing-team/           ← 플러그인 본체 (설치되는 것은 이 폴더다)
  ...
  skills/       마케팅팀-시작하기 · 스킬-100-보기 · 마케팅-CMO
