...
> **스킬 100개를 갖춘 마케팅 팀 하나.** 설치하면 바로 쓸 수 있다.
...
| | 하는 일 | 결과 |
|---|---|---|
| **①** | 설치하고 **「마케팅 팀 업무 시작하자」** | 스킬 100개를 **바로 씁니다.** 번호도 이름도 몰라도 됩니다 |
| **②** | **`marketing-team-setup`**으로 우리 회사 정보 | 100개가 **우리 값으로** 돕니다. 3분 |
...
```
마케팅 팀 업무 시작하자    ← 무엇을 시킬지 모를 때
리뷰 분석해줘            경쟁사 비교해줘        이번 달 광고 어땠어
상세페이지 써줘           예산 다시 짜줘         광고애널리틱스 돌려줘
```

**명령어를 외우지 않는다.** 하고 싶은 일을 그대로 말하면 알맞은 스킬이 열린다.
딱 맞는 말이 아니어도 된다. 비슷하게 말해도 찾아내고, 그래도 안 되면 무엇을 하려는지 되묻는다.

**스킬 이름은 「무엇을 + 어떻게」 형식이다.** 끝말이 여덟 가지뿐이라 목록을 훑을 때 종류가 바로 구분된다.

| 끝말 | 무엇을 하는 스킬인가 | 예 |
|---|---|---|
| **조사** | 밖에서 자료를 모아 본다 | 키워드 수요 조사 · 가격 조사 |
| **분석** | 있는 자료를 나누어 계산하고 결론을 낸다 | 소비자 리뷰 분석 · 판매 데이터 분석 |
...
| **정리** | 흩어진 것을 모아 종류별로 묶는다 | 고객 의견 정리 · 회의록 정리 |
...
설치하면 **Skills 탭에 3개, Agents 탭에 3개**가 나타난다.
그다음 **`marketing-team-setup`**을 누른다.
...
**서브에이전트는 「독립성이 필요한 검토」에만 씁니다.** 실행은 전부 메인에서 합니다.
...
| 무엇을 | 어떻게 |
|---|---|
| **필수 재료** | 계획 화면 **한 곳에 모아** 묻고, **승인 한 답으로** 받습니다. 예: 「진행 승인. 리뷰는 inputs/우리리뷰.csv 로」 |
| **산출 형식** | 승인 **전에** 묻습니다. 만든 뒤에 「이 형식이 아닌데」가 되면 다시 만들어야 하니까요 |
| **바깥 연결** | MCP·웹 조사·GA4 가 붙어 있으면 실측으로, 없으면 연습용 자료로 돌고 **어느 쪽인지 밝힙니다** |
| **공식 스킬** | `.pptx` 처럼 우리가 못 만드는 형식은 **공식 스킬에 넘깁니다.** 없으면 `.md` 로 내고 그렇게 말합니다 |
...
자리마다 **바로 쓸 수 있는 생성 프롬프트**를 드리고, 만드는 일은 쓰시는 이미지 도구가 합니다.
...
**멈추는 자리가 32곳.** 대외로 나가는 산출물 32개는 발행 전 규제 검사를 통과해야 한다.
표시광고법이 공통이고, `brand/profile.md` 의 업종에 따라 화장품법이나 건강기능식품법이 자동으로 갈아 끼워진다.
⛔ 가 나오면 **전달하지 않도록 지시한다.** 다만 **기술적 강제 차단은 아니다.**
...
우리는 **내용을 만들고**, 파일로 만들거나 읽는 일만 넘긴다. **대부분 이미 깔려 있다.**
...
| 몇 개 | 하나 | **여러 개** (브랜드·프로젝트별로 따로 둘 수 있다) |
...
**데스크탑처럼 다른 게 이미 많은 곳이면** 「마케팅 팀 구축하자」가 `marketing-team/` 폴더를
