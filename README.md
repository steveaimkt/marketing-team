...
처음 설치했다면 → **「마케팅 팀 구축하자」**
무엇을 맡길 수 있는지 보려면 → **「마케팅 팀 업무 리스트 보여줘」**
업무를 맡기려면 → **「마케팅 팀 업무 시작하자」**, 또는 바로 「리뷰 분석해줘」
팀 단위로 맡기려면 → **「브랜드 마케팅 팀 업무 시작하자」** (콘텐츠, 퍼포먼스, 그로스도 같은 형식이다)

스킬 번호나 이름은 외울 필요가 없다. 사용자가 하고 싶은 일을 평소 말로 입력하면 AI 마케터가 알맞은 스킬을 찾아 실행한다.

## 무엇이 다른가

일반 AI 대화는 답을 글로 돌려준다. 마케팅 팀은 정해진 절차에 따라 업무를 실행하고 결과를 파일로 남긴다.

- **스킬마다 절차가 정해져 있다.** 읽을 자료, 사용자에게 확인할 지점, 저장할 파일이 스킬 100개에 각각 지정되어 있다. 같은 요청을 다시 입력하면 같은 형식의 결과가 나온다.
- **실행 전에 계획을 확인받는다.** AI 마케터는 만들 결과물과 사용할 자료, 저장 위치를 한 화면에 보여 주고, 사용자가 「진행 승인」이라고 답한 뒤에 실행한다.
...
사용자 (모든 ⏸ 승인을 최종 결정한다)
   │
AI 마케터         요청을 받아 계획을 보여 주고, 승인받은 뒤 스킬 100개를 직접 실행한다
   │
   ├─ AI 규제검토자   외부에 공개되는 글의 표현을 검토한다 · ⛔ 결과는 AI 마케터도 바꿀 수 없다
   ├─ AI 사업검토자   경영·재무·고객·법무·브랜드 다섯 관점으로 중요한 판단을 검토한다
   └─ AI 규제세팅     기본 사전에 없는 업종의 「조심할 말 목록」을 만든다
```

| 구성 | 내용 |
|---|---|
| **진입 스킬 3개** | `ai-marketer`, `marketing-team-setup`, `marketing-team-tasks` |
| **업무 스킬 100개** | 메뉴에 올리는 대신, AI 마케터가 명부에서 요청에 맞는 스킬을 찾아 실행한다 |
| **체인 17종** | 자주 쓰는 실행 순서를 미리 묶어 두었다. 「시장리서치 돌려줘」라고 입력하면 키워드 조사부터 가격 조사까지 4단계가 이어서 실행된다 |
| **담당 3명** | 제작과 검토 역할을 분리했다. AI 마케터가 만든 결과는 다른 담당이 검토한다 |

AI 마케터는 모든 업무를 같은 순서로 진행한다. **무엇을 할지 묻고 → 필요한 자료를 알려 주고 → 계획을 승인받고 → 실행하고 → 결과를 보고하고 → 다음 업무를 제안한다.**

| | AI 규제검토자 | AI 사업검토자 |
|---|---|---|
| 확인하는 것 | **법에 저촉되는가** | **사업적으로 타당한가** |
| 대상 | 외부에 공개되는 산출물 32개 | 런칭 플랜, 제안서, 예산안처럼 사업적 판단이 필요한 산출물 |
| 예 | 「업계 1위」라는 표현에 실증자료가 없다 → ⛔ 발행할 수 없다 | 할인율이 30%면 마진이 8%다 → 발행은 가능하지만 팔수록 손해다 |
...
| 도구 | 무엇이 달라지나 | 없으면 |
|---|---|---|
| **브라우저 도구** (`@playwright/mcp`, 플러그인과 함께 온다) | 키워드 조사, 경쟁사 조사, 트렌드 조사, 소셜 언급 분석이 공개 웹 페이지를 직접 읽는다 | 사용자가 자료를 직접 넣는 방식으로 그대로 작동한다 |
...
⚠️ 한글(HWP) 파일은 PDF나 워드로 저장한 뒤 넣는다.

## 설치하면 내 컴퓨터에 무엇이 생기나

**플러그인과 사용자의 데이터는 서로 다른 곳에 저장된다.** 결과물을 찾을 때는 작업 폴더를 연다.
