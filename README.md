...
| | 하는 일 | 결과 |
|---|---|---|
| **①** | 설치하고 **`0-사용가이드`** | 스킬 100개를 **바로 씁니다.** 「리뷰 분석해줘」 하면 됩니다 |
...
| 스킬 | 언제 |
|---|---|
| `0-사용가이드` | **처음 열었을 때** · 3분 안내 + 첫 결과물 |
| `1-시작하기` | 설치 직후 점검 · 작업 폴더 생성 · 안 되면 고친다 |
...
작업할 폴더를 하나 열고 **`0-사용가이드`**. 그 폴더에 `brand/` · `outputs/` · `logs/` 를 만들어 준다.
...
plugins/marketing-team/           ← 플러그인 본체 (설치되는 것은 이 폴더다)
  ...
  brand-templates/  빈 템플릿 원본 + skill-notes 템플릿 — 작업 폴더의 brand/ 와 이름이 겹치면 안 된다
