...
| **②** | **`marketing-team-setup`** 로 우리 회사 정보 | 100개가 **우리 값으로** 돕니다. 3분 |
...
| 스킬 | 언제 |
|---|---|
| `marketing-team-setup` | 설치 직후 점검 · 작업 폴더 생성 · 브랜드 정보 등록 |
| `marketing-team-tasks` | 무엇을 시킬 수 있는지 볼 때 |
| **`ai-marketer`** | **실제 업무를 받는 자리** · 매일 쓰는 입구 |
...
그다음 **`marketing-team-setup`** 를 누른다.
...
작업할 폴더를 하나 열고 **`marketing-team-setup`**. 그 폴더에 `brand/` · `outputs/` · `logs/` 를 만들어 준다.
...
> 그러면 담당이 하나도 안 걸린다. `marketing-team-setup` 가 그것을 찾아 그 자리에서 고친다.
