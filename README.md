...
### 그래서 세 모드가 있습니다 · 채운 만큼 올라갑니다

| 모드 | 켜지는 조건 | 어떻게 도나 |
|---|---|---|
| **① 스킬** | 기본 | 혼자 스킬 하나를 완주한다. 샘플로 돈다 |
| **② 내 에이전트** | 브랜드 정보를 채우면 **자동 조립** | 우리 스킬·체인·기본값으로 돈다. 실적이 쌓인다 |
| **③ 팀** | `4-팀-짜기` (선택) | 주전 팀장에게 위임하고, 발행물은 감사관을 거친다 |

**「어떤 모드로 쓰시겠어요?」를 묻지 않습니다.** ③은 필요해질 때만 갑니다 —
여러 사람이 나눠 맡거나, 판정이 갈려 독립 검수가 필요할 때입니다.
**편성하지 않은 팀장은 한 번도 호출되지 않습니다.**
...
설치하면 **Skills 탭에 6개, Agents 탭에 14명**이 뜬다.
그다음 **`1-시작하기`** 를 누른다.
