...
설치하면 담당 14명이 뜨지만 **처음부터 팀으로 일하지는 않습니다.** 모드는 파일이 정합니다.

| 모드 | 켜지는 조건 | 어떻게 도나 |
|---|---|---|
| **① 스킬** | 기본 | 디렉터가 **혼자** 스킬 하나를 완주한다. 샘플로 돈다 |
| **② 내 에이전트** | `2-브랜드-등록하기` | 혼자 하되 **내 데이터로**, 실적이 쌓이고 다음이 이어진다 |
| **③ 팀** | `3-팀-짜기` | 주전 팀장에게 **위임**하고, 발행물은 감사관을 거친다 |

**「어떤 모드로 쓰시겠어요?」를 묻지 않습니다.** 채우면 올라갑니다.
그리고 **편성하지 않은 팀장은 한 번도 호출되지 않습니다** — 만들지 않은 조직이 일하지 않습니다.
