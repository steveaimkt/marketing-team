...
| | 무엇을 하나 | 끝나는 조건 |
|---|---|---|
| **1-시작하기** | 15명이 실제로 떴는지 세고, 작업 공간을 만들고, 방법론 하나를 같이 돌린다 | 첫 산출물 파일이 생기면 |
| **2-브랜드-등록하기** | 우리 회사 정보를 넣는다 | 산출물이 `[샘플]` → `[실데이터]` 로 바뀌면 |
| **3-팀-짜기** | 지금 하는 업무에 맞는 팀만 켠다 | 편성 승인 + 첫 업무가 실제로 돌면 |
| **방법론-100-보기** | 100개를 펼쳐 보여준다 | 언제든 · 뭘 시킬 수 있는지 볼 때 |
...
설치하면 **Skills 탭에 4개, Agents 탭에 15명**이 뜬다.
그다음 **`1-시작하기`** 를 누른다.
...
연결돼 있어 그대로 뜬다. 방법론 파일과 산출물이 **한 폴더에 다 보이는** 것이 장점이라 책 실습에는 이쪽이 낫다.
...
plugins/marketing-team/           ← 플러그인 본체 (설치되는 것은 이 폴더다)
  ...
  skills/       방법론-100-보기 · 3-팀-짜기 · 1-시작하기 · 2-브랜드-등록하기
